...
def log_metrics():
    while True:
        for client_id in ["xeno-client1-1", "xeno-client2-1"]:  # Example client IDs
            ...
            # Extract speed from the metrics dictionary
            speed = metrics['bandwidth']  # Access the bandwidth from the dictionary
            
            # Log metrics to the database
            c.execute("INSERT INTO metrics (client_id, speed, timestamp) VALUES (?, ?, ?)", (client_id, speed, timestamp))
            conn.commit()
            
            print(f"{timestamp} - Bandwidth for {client_id}: {speed} Mbps")  # Optional: Keep if you want to see it
...
def get_current_speed(client_id):
    ...
    try:
        ...
        # Print the entire output for debugging (comment out if not needed)
        ...
        for line in result.stdout.splitlines():
            if "Mbits/sec" in line:
                try:
                    ...
                    return {
                        "client_id": client_id,
                        "bandwidth": speed,  # Renamed to 'bandwidth'
                        "unit": "Mbps",      # Changed unit to Mbps
        ...
        print(f"An error occurred while measuring speed for {client_id}: {e}")
        return {
            "client_id": client_id,
            "bandwidth": 0.0,
            "unit": "Mbps",
    ...
    return {
        ...
        "unit": "Mbps",
        "timestamp": datetime.now().isoformat()
    }  # Default value if speed measurement fails
