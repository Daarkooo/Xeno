...
train_data = pd.read_csv('optim_train_set.csv',sep=',')
...
def evaluate_model(data):
    ...
    for episode in range(10):  # Evaluate the model for 10 episodes
        obs = env.reset()
        done = False
        episode_reward = 0
        episode_efficiency_reward = 0
        episode_over_allocation_penalty = 0
        episode_abusive_usage_penalty = 0
        
        print(f"Evaluating episode {episode + 1}")
        ...
        while not done:
            ...
            episode_efficiency_reward += info['efficiency_reward']
            episode_over_allocation_penalty += info['over_allocation_penalty']
            episode_abusive_usage_penalty += info['abusive_usage_penalty']
            step_rewards.append(reward)
        
        total_reward += episode_reward
        total_efficiency_reward += episode_efficiency_reward
        total_over_allocation_penalty += episode_over_allocation_penalty
        total_abusive_usage_penalty += episode_abusive_usage_penalty
        
        print(f"Episode {episode + 1} - Total Reward: {episode_reward:.2f}, "
              f"Efficiency Reward: {episode_efficiency_reward:.2f}, "
              f"Over-Allocation Penalty: {episode_over_allocation_penalty:.2f}, "
              f"Abusive Usage Penalty: {episode_abusive_usage_penalty:.2f}")
    
    average_efficiency = total_efficiency_reward / (288 * 10)
    
    print("\nFinal Evaluation Summary:")
    print(f"Total Reward: {total_reward:.2f}")
    print(f"Average Efficiency Reward: {average_efficiency:.2f}")
    print(f"Total Over-Allocation Penalty: {total_over_allocation_penalty:.2f}")
    print(f"Total Abusive Usage Penalty: {total_abusive_usage_penalty:.2f}")
